...
uint8 UART_net_initVar = 0u;
    ...
    volatile uint8 UART_net_txBuffer[UART_net_TX_BUFFER_SIZE];
    ...
    uint8 UART_net_errorStatus = 0u;
    volatile uint8 UART_net_rxBuffer[UART_net_RX_BUFFER_SIZE];
    ...
    volatile uint8 UART_net_rxBufferOverflow   = 0u;
        ...
        volatile uint8 UART_net_rxAddressMode = UART_net_RX_ADDRESS_MODE;
...
*  UART_net_Start() sets the initVar variable, calls the
*  UART_net_Init() function, and then calls the
*  UART_net_Enable() function.
*
* Parameters:
*  None.
*
...
*  and set to one (1u) the first time UART_net_Start() is called. This
*  allows for component initialization without re-initialization in all
*  subsequent calls to the UART_net_Start() routine.
*
* Reentrant:
*  No.
*
...
*  dialog settings. It is not necessary to call UART_net_Init() because
*  the UART_net_Start() API calls this function and is the preferred
...
void UART_net_Init(void) 
{
            ...
            UART_net_errorStatus = 0u;
        #endif /* (UART_net_RX_INTERRUPT_ENABLED) */

        #if (UART_net_RXHW_ADDRESS_ENABLED)
            UART_net_SetRxAddressMode(UART_net_RX_ADDRESS_MODE);
            UART_net_SetRxAddress1(UART_net_RX_HW_ADDRESS1);
            UART_net_SetRxAddress2(UART_net_RX_HW_ADDRESS2);
...
*  to call UART_net_Enable() because the UART_net_Start() API
...
void UART_net_Enable(void) 
{
    ...
    enableInterrupts = CyEnterCriticalSection();
        ...
        #if (UART_net_TX_INTERRUPT_ENABLED)
            UART_net_ClearPendingTxInt(); /* Clear history of TX_NOT_EMPTY */
            UART_net_EnableTxInt();
        #endif /* (UART_net_TX_INTERRUPT_ENABLED) */
     #endif /* (UART_net_TX_INTERRUPT_ENABLED) */

    #if (UART_net_INTERNAL_CLOCK_USED)
        UART_net_IntClock_Start();  /* Enable the clock */
    #endif /* (UART_net_INTERNAL_CLOCK_USED) */
...
void UART_net_Stop(void) 
{
    ...
    #if (UART_net_INTERNAL_CLOCK_USED)
        UART_net_IntClock_Stop();   /* Disable the clock */
    #endif /* (UART_net_INTERNAL_CLOCK_USED) */
...
uint8 UART_net_ReadControlRegister(void) 
{
    ...
    #endif /* (UART_net_CONTROL_REG_REMOVED) */
}
...
void  UART_net_WriteControlRegister(uint8 control) 
{
    ...
    *      UART_net_RX_STS_FIFO_NOTEMPTY    Interrupt on byte received.
    *      UART_net_RX_STS_PAR_ERROR        Interrupt on parity error.
    *      UART_net_RX_STS_STOP_ERROR       Interrupt on stop error.
    *      UART_net_RX_STS_BREAK            Interrupt on break.
    *      UART_net_RX_STS_OVERRUN          Interrupt on overrun error.
    *      UART_net_RX_STS_ADDR_MATCH       Interrupt on address match.
    *      UART_net_RX_STS_MRKSPC           Interrupt on address detect.
    *
    * Return:
    *  None.
    *
    ...
    *  UART_net_rxBufferLoopDetect - cleared if loop condition was detected
    *     in RX ISR.
    *
    * Reentrant:
    *  No.
    ...
    *******************************************************************************/
    uint8 UART_net_ReadRxData(void) 
    {
        ...
        locRxBufferRead  = UART_net_rxBufferRead;
        locRxBufferWrite = UART_net_rxBufferWrite;

        if( (UART_net_rxBufferLoopDetect != 0u) || (locRxBufferRead != locRxBufferWrite) )
        {
            rxData = UART_net_rxBuffer[locRxBufferRead];
            locRxBufferRead++;

            if(locRxBufferRead >= UART_net_RX_BUFFER_SIZE)
            {
                locRxBufferRead = 0u;
            }
            /* Update the real pointer */
            UART_net_rxBufferRead = locRxBufferRead;
            ...
            if(UART_net_rxBufferLoopDetect != 0u)
            {
                ...
                #endif /* ((UART_net_RX_INTERRUPT_ENABLED) && (UART_net_FLOW_CONTROL != 0u)) */
            }
        }
        else
    ...
    *  UART_net_RX_STS_FIFO_NOTEMPTY.
    *  UART_net_RX_STS_FIFO_NOTEMPTY clears immediately after RX data
    *  register read.
    *
    * Global Variables:
    ...
    *
    * Summary:
    *  Returns the last received byte of data. UART_net_GetChar() is
    ...
    *******************************************************************************/
    uint16 UART_net_GetByte(void) 
    {
        ...
        /* Protect variables that could change on interrupt */
        UART_net_DisableRxInt();
        locErrorStatus = (uint16)UART_net_errorStatus;
        UART_net_errorStatus = 0u;
        UART_net_EnableRxInt();
        return ( (uint16)(locErrorStatus << 8u) | UART_net_ReadRxData() );
    #else
    ...
    *******************************************************************************/
                                                            ...
                                                            
    {
        ...
        {
            size = (UART_net_RX_BUFFER_SIZE - UART_net_rxBufferRead) + UART_net_rxBufferWrite;
        }
    ...
    *   UART_net_TX_STS_COMPLETE        Interrupt on TX byte complete
    *   UART_net_TX_STS_FIFO_EMPTY      Interrupt when TX FIFO is empty
    *   UART_net_TX_STS_FIFO_FULL       Interrupt when TX FIFO is full
    *   UART_net_TX_STS_FIFO_NOT_FULL   Interrupt when TX FIFO is not full
    *
    * Return:
    *  None.
    *
    ...
    *******************************************************************************/
    void UART_net_WriteTxData(uint8 txDataByte) 
    {
        ...
        if(UART_net_initVar != 0u)
        {
            ...
            {
                if(UART_net_txBufferWrite >= UART_net_TX_BUFFER_SIZE)
                {
    ...
    *******************************************************************************/
    void UART_net_PutChar(uint8 txDataByte) 
    {
        ...
        */
        ...
        #if ((UART_net_TX_BUFFER_SIZE > UART_net_MAX_BYTE_VALUE) && (CY_PSOC3))
            /* Disable TX interrupt to protect variables from modification */
            UART_net_DisableTxInt();
        #endif /* (UART_net_TX_BUFFER_SIZE > UART_net_MAX_BYTE_VALUE) && (CY_PSOC3) */

            locTxBufferWrite = UART_net_txBufferWrite;
            locTxBufferRead  = UART_net_txBufferRead;
        ...
        while( (locTxBufferWrite < locTxBufferRead) ? (locTxBufferWrite == (locTxBufferRead - 1u)) :
                                ...
                                (uint8)(UART_net_TX_BUFFER_SIZE - 1u)) );
        ...
        {
            if(locTxBufferWrite >= UART_net_TX_BUFFER_SIZE)
            {
                locTxBufferWrite = 0u;
            }
            /* Add to the software buffer. */
            UART_net_txBuffer[locTxBufferWrite] = txDataByte;
            locTxBufferWrite++;
            ...
            UART_net_txBufferWrite = locTxBufferWrite;

        #if ((UART_net_TX_BUFFER_SIZE > UART_net_MAX_BYTE_VALUE) && (CY_PSOC3))
            UART_net_EnableTxInt();
        #endif /* (UART_net_TX_BUFFER_SIZE > UART_net_MAX_BYTE_VALUE) && (CY_PSOC3) */

            if(0u != (UART_net_TXSTATUS_REG & UART_net_TX_STS_FIFO_EMPTY))
            {
                /* Trigger TX interrupt to send software buffer */
                UART_net_SetPendingTxInt();
            }
        }
    ...
    *******************************************************************************/
    void UART_net_PutString(const char8 string[]) 
    {
        ...
        if(UART_net_initVar != 0u)
        {
            ...
            {
                UART_net_PutChar((uint8)string[bufIndex]);
                bufIndex++;
            }
        }
    }
    ...
    *******************************************************************************/
                                                                    ...
                                                                    
    {
        ...
        if(UART_net_initVar != 0u)
        {
            ...
            {
                UART_net_PutChar(string[bufIndex]);
                bufIndex++;
            }
        }
    }
    ...
    *******************************************************************************/
                                                            ...
                                                            
    {
        ...
        {
            size = (UART_net_TX_BUFFER_SIZE - UART_net_txBufferRead) +
                    UART_net_txBufferWrite;
        }
    ...
    *   UART_net_SEND_BREAK - Initialize registers for break, send the Break
    *       signal and return immediately.
    *   UART_net_WAIT_FOR_COMPLETE_REINIT - Wait until break transmission is
    *       complete, reinitialize registers to normal transmission mode then return
    *   UART_net_REINIT - Reinitialize registers to normal transmission mode
    *       then return.
    *   UART_net_SEND_WAIT_REINIT - Performs both options: 
    *      UART_net_SEND_BREAK and UART_net_WAIT_FOR_COMPLETE_REINIT.
    ...
    *     SendBreak(0);     - initialize Break signal transmission
    *         Add your code here to use CPU time
    *     When interrupt appear with UART_net_TX_STS_COMPLETE status:
    *     SendBreak(2);     - complete Break operation
    *
    * Side Effects:
    *  The UART_net_SendBreak() function initializes registers to send a
    *  break signal.
    ...
    void UART_net_SendBreak(uint8 retMode) 
    {
        ...
        if(UART_net_initVar != 0u)
        {
                ...
                (retMode == UART_net_SEND_WAIT_REINIT) )
            {
                ...
                #if(UART_net_TXCLKGEN_DP)
                    txPeriod = UART_net_TXBITCLKTX_COMPLETE_REG;
                    UART_net_TXBITCLKTX_COMPLETE_REG = UART_net_TXBITCTR_BREAKBITS;
                #else
                    txPeriod = UART_net_TXBITCTR_PERIOD_REG;
            ...
            #if(UART_net_TXCLKGEN_DP)
                UART_net_TXBITCLKTX_COMPLETE_REG = txPeriod;
            #else
                UART_net_TXBITCTR_PERIOD_REG = txPeriod;
            #endif /* End UART_net_TXCLKGEN_DP */
    ...
    * Parameters:
    *  addressMode: 
    *       UART_net_SET_SPACE - Configure the transmitter to send the next
    *                                    byte as a data.
    *       UART_net_SET_MARK  - Configure the transmitter to send the next
    *                                    byte as an address.
    *
    * Return:
    *  None.
    *
    * Side Effects:
    *  This function sets and clears UART_net_CTRL_MARK bit in the Control
    *  register.
    *
    *******************************************************************************/
